...
// TODO: specify appropriate passport strategy module (http://passportjs.org/) and declare it as a dependency
var dataSourcePassportStrategy = require('passport-oauth2').Strategy; 
...
function sampleConnector( parentDirPath ){
	...
	var connectorInfo = {
						  id: 'oa2_template',			        // internal connector ID 
						  name: 'OA2 OAuth Data Source'	// connector display name
						};
	...
	/**
	 * the passport strategy to use for authentication
	 */
	this.getPassportStrategy = function(pipe) {
		return new dataSourcePassportStrategy({
    		...
    		customHeaders: {'User-Agent' : 'Simple Data Pipe test app 0.1'}
		  },
		  function(accessToken, refreshToken, profile, done) {
			  process.nextTick(function () {
			  	...
			  	console.log('Secret: ' + refreshToken);
			  	console.log('Profile: ' + require('util').inspect(profile));
	 ...
	 */
	this.authCallback = function( accessToken, pipeId, callback ){
		...
		pipesDb.getPipe( pipeId, function( err, pipe ){
			...
			// fetch list of data sets to choose from
           	// in the pipe configuration
			pipe.tables = getTables();
